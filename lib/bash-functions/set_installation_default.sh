...
function set_installation_default {
  ...
  tmp_file="$(mktemp "$(dirname "$CONFIG_INSTALLATIONS_JSON_FILE")/.installations.json.XXXXXX")"
  jq -n --arg name "$name" '{default: $name}' > "$tmp_file"
  mv "$tmp_file" "$CONFIG_INSTALLATIONS_JSON_FILE"
}
