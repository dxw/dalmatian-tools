...
@test "setup -f onto an existing installation refuses a file naming another project" {
  mkdir -p "$CONFIG_INSTALLATIONS_DIR/client-a"
  cp "$SANDBOX/setup.json" "$CONFIG_INSTALLATIONS_DIR/client-a/setup.json"
  jq '.project_name = "other-project"' "$SANDBOX/setup.json" > "$SANDBOX/other.json"

  run --separate-stderr run_command bin/configure-commands/v2/setup -f "$SANDBOX/other.json" -n client-a < "$SANDBOX/answers"
  assert_failure 1
...
@test "announces an existing installation before the first prompt" {
  ...
  assert_output_contains "To set up a different project instead, stop now and run \`dalmatian setup -n <new-installation-name>\`"
}
