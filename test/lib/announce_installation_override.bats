...
@test "announce_installation_override is silent when the installation is the default" {
  ...
  run --separate-stderr announce_installation_override
  assert_success
  assert_output ""
  [ -z "$stderr" ]
}
...
@test "announce_installation_override names both installations when the environment overrides the default" {
  ...
  run --separate-stderr announce_installation_override
  assert_success
  assert_stderr_contains "Using Dalmatian installation 'other'"
  assert_stderr_contains "overrides the default 'example-project'"
}
...
@test "announce_installation_override says so when no default is recorded" {
  ...
  assert_success
  assert_stderr_contains "Using Dalmatian installation 'other'"
  assert_stderr_contains "no default is recorded"
}

@test "announce_installation_override prints even in quiet mode" {
  printf '%s\n' '{"default": "example-project"}' > "$CONFIG_INSTALLATIONS_JSON_FILE"
  export DALMATIAN_INSTALLATION=other
  export QUIET_MODE=1
...
@test "announce_installation_override prints once per process tree" {
  ...
  export -f announce_installation_override installation_default warning
