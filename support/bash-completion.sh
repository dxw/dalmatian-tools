...
_dalmatian_completion() {
  ...
  then
    ...
    # -y is only parsed before the name, so only offer it there
    elif [[ "${#COMP_WORDS[@]}" == 4 && "${COMP_WORDS[2]}" == "remove" && "${COMP_WORDS[COMP_CWORD]:0:1}" == "-" ]]
    then
      while IFS='' read -r comp
      do
        COMPREPLY+=("$comp")
      done < <(compgen -W "-y" -- "${COMP_WORDS[COMP_CWORD]}")
