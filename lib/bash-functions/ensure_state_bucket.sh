...
function ensure_state_bucket {
  ...
  then
    ...
    if [ "$(aws s3api get-bucket-versioning --bucket "$BUCKET" --profile dalmatian-main --region "$REGION" --query 'Status' --output text 2>/dev/null)" != "Enabled" ]
    then
      MISSING+=("versioning")
    fi
    if ! aws s3api get-public-access-block --bucket "$BUCKET" --profile dalmatian-main --region "$REGION" --query 'PublicAccessBlockConfiguration.[BlockPublicAcls,IgnorePublicAcls,BlockPublicPolicy,RestrictPublicBuckets]' --output text 2>/dev/null | grep -qx $'True\tTrue\tTrue\tTrue'
    then
      MISSING+=("public access block")
    fi
    if ! aws s3api get-bucket-encryption --bucket "$BUCKET" --region "$REGION" --profile dalmatian-main > /dev/null 2>&1
    then
